...
if(count($url) == 3){
  ...
  $post_count = countContent2($conn, "web_content", "hash_id", $id);
  if($post_count < 1){
    header("Location: /");
    exit();
  }else{
...
$comment_count = countContent2($conn, "hom_comments", "content_id", $post['hash_id']);
$replies_count = countContent2($conn, "hom_replies", "content_id", $post['hash_id']);
...
if($visitor_ip_check > 0){
  $chk = ipCheck($conn, $post['hash_id'], $visitor_ip, "NULL");

  if($chk->rowCount() < 1){
    visitPage($conn, "stats", $post['hash_id'], $visitor_ip, "NULL");
    $hits = $conn->prepare("UPDATE web_content SET hits = hits+1 WHERE hash_id = :id ");
    $hits->bindParam(":id", $post['hash_id']);
    $hits->execute();
  }else{
    if(isset ($_SESSION['user'])){
      $ip_user_check = ipCheck($conn, $post['hash_id'], $visitor_ip, $_SESSION['user']);
      if($ip_user_check->rowCount() < 1){
        visitPage($conn, "stats", $post['hash_id'], $visitor_ip, $_SESSION['user']);
        $hits = $conn->prepare("UPDATE web_content SET hits = hits+1 WHERE hash_id = :id ");
        $hits->bindParam(":id", $post['hash_id']);
        $hits->execute();

        $desc = "Viewed a post :-";
        $desc_post = $post['title'];
        $desc.=" $desc_post";
...
// if($visitor_ip_check->rowCount() < 1 &&){
//   visitPage($conn, "stats", $post['hash_id'], $visitor_ip, $_SESSION['user']);
//   $hits = $conn->prepare("UPDATE web_content SET hits = hits+1 WHERE hash_id = :id ");
          ...
          <div class="post-item">
            <div class="post-item-wrap">
              <div class="post-image">
                <a href="#">
                  <img alt="" src="/uploads/POSTS/<?php echo $post['image'] ?>">
                </a>
              </div>
              <div class="post-item-description">
                <h2><?php echo $post['title'] ?></h2>
                <div class="post-meta">
                  <span class="post-meta-date"><i class="fa fa-calendar-o"></i>Posted - <span><time class="timeago" datetime="<?php echo $post['date_created'] ?> <?php echo $post['time_created'] ?>"></time></span> on <?php echo date("F j, Y", strtotime($post['date_created'])) ?>
                  </span>
                  ...
                  <span class="post-meta-category">
                    <a href="/category/<?= $post_section_slug[$post['section']] ?>-<?= $post['section'] ?>"><i class="fa fa-tag">
                    </i><?php echo $post_section_data[$post['section']] ?>
                    </a>,
                    <a href="/category/<?= $post_section_slug[$post['section']] ?>/<?= $post_category_slug[$post['category_id']] ?>-<?php echo $post['category_id'] ?>"><?php echo $post_category_data[$post['category_id']] ?>
                    </a>
              ...
              <div class="post-navigation">
                <?php $prev = $post['web_content_id'] - 1;
                $count_prev_post = countContent2($conn, "web_content", "web_content_id", $prev);
                $previous_post = fetchRecord2($conn, "web_content", "web_content_id", $prev);
                 ?>
                <?php $next = $post['web_content_id'] + 1;
                $count_next_post = countContent2($conn, "web_content", "web_content_id", $next);
                $next_post = fetchRecord2($conn, "web_content", "web_content_id", $next);
                 ?>
                <?php if($count_prev_post > 0): ?>
                  <a href="/post/<?=$previous_post['title_slug']?>-<?= $previous_post['hash_id'] ?>" class="post-prev">
                    <div class="post-prev-title">
                      <span>Previous Post</span><?= $previous_post['title']?>
                    </div>
                  </a>
                <?php endif ?>
                ...
                <?php if($count_next_post > 0): ?>
                  <a href="/post/<?=$next_post['title_slug']?>-<?= $next_post['hash_id'] ?>" class="post-next">
                    <div class="post-next-title"><span>Next Post</span><?= $next_post['title']?></div>
                  </a>
                <?php endif ?>
              </div>
              ...
              <div class="comments" id="comments">
                ...
                <div class="comment-list">
                  <div id="loadcomment">
                    <?php $comment = loadRecord($conn, "hom_comments", "content_id", $post['hash_id'], "comment_id", 2) ?>
                    <?php foreach ($comment as $key => $value): ?>
                      <div class="comment" id="comment-1">
                        ...
                        <div class="text">
                          ...
                          <a class="comment-reply-link" href="javascript:void(0)" onclick="getReply('<?= $_SERVER['REQUEST_URI'] ?>', '<?= $value['comment_id'] ?>', '<?= $post['hash_id'] ?>', '<?= $value['user_id'] ?>')">Reply</a>
                ...
                <div class="section-row loadmore text-center">
                  <button onclick="loadComment('/ajax_lc', '<?= $post['hash_id'] ?>')" class="btn">Load More</button>
                </div>
              <?php endif ?>
              </div>
                      ...
                      <div class="row">
                        <div class="col-lg-12">
                          <div class="form-group text-center">
                            <button class="btn" type="button" onclick="submitComment('<?= $_SERVER['REQUEST_URI'] ?>', '<?= $post['hash_id'] ?>', '<?= $_SESSION['user'] ?>')">Submit Comment</button>
                          </div>
                        </div>
                      </div>
                    </form>
