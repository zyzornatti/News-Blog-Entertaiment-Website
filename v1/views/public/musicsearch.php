...
if(isset ($_GET['q'])){
  $query = 'SELECT hom_mav.hom_mav_id, hom_mav.hom_mav_hash_id, hom_mav.hom_mav_title, hom_mav.hom_mav_title_slug, hom_mav.hom_mav_category_id, hom_mav.hom_mav_added_by, admin.admin_username AS author, admin.admin_profile_pics, hom_mav_category.hom_mav_category_name AS category, hom_mav_category.hom_mav_category_slug, hom_mav.hom_mav_content AS body, hom_mav.hom_mav_image, hom_mav.date_created, hom_mav.time_created FROM hom_mav INNER JOIN hom_mav_category ON hom_mav.hom_mav_category_id = hom_mav_category.hom_mav_category_hash_id INNER JOIN admin ON hom_mav.hom_mav_added_by = admin.admin_hash ';

  $query .= 'WHERE hom_mav.hom_mav_title LIKE "%'.str_replace(' ', '%', $_GET['q']).'%" OR admin.admin_username LIKE "%'.str_replace(' ', '%', $_GET['q']).'%" OR hom_mav_category.hom_mav_category_name LIKE "%'.str_replace(' ', '%', $_GET['q']).'%" OR hom_mav.hom_mav_content LIKE "%'.str_replace(' ', '%', $_GET['q']).'%" ';
      ...
      <div class="content col-lg-9">
        ...
        <div class="page-title">
         ...
         <div class="breadcrumb float-left">
           ...
           <li><a href="#">Music Search result</a>
           </li>
           <li><h3 href="#"><?= $paginated_data ?> of <?= $total_data ?> music(s) found</h3>
           </li>
           </ul>
         </div>
        </div>
             ...
             <div class="post-item">
               <div class="post-item-wrap">
                 <div class="post-image">
                   ...
                   <span class="post-meta-category"><a href="/musiccategory/<?= $value['hom_mav_category_slug'] ?>-<?php echo $value['hom_mav_category_id'] ?>"><?php echo $value['category'] ?></a></span>
                 </div>
                 <div class="post-item-description">
                   ...
                   <p>by <a href="/author/<?= strtolower($value['author']) ?>-<?= $value['hom_mav_added_by'] ?>"><?php echo $value['author'] ?></a> <time class="timeago" datetime="<?php echo $value['date_created'] ?> <?php echo $value['time_created'] ?>"></p>
                   </div>
                   <?php endif ?>
                 </div>
               </div>
                ...
                ';
                 ...
                 for ($count = 0; $count < count($page_array); $count++){
                  if($page == $page_array[$count]){
                    ...
                    $previous_id = $page_array[$count] - 1;
                         if($previous_id > 0){
                           $previous_link.= '
                           <li class="page-item">
                             <a class="page-link" href="/msearch?q='.$_GET['q'].'&page='.$previous_id.'"><i class="fa fa-angle-left"></i></a>
                           </li>
                           ';
                          }else{
                            $previous_link.= '
                        ...
                        $next_id = $page_array[$count] + 1;
                         if($next_id >= $total_links){
                           ...
                           $next_link.= '
                           <li class="page-item">
                             <a class="page-link" href="/msearch?q='.$_GET['q'].'&page='.$next_id.'"><i class="fa fa-angle-right"></i></a>
                           </li>
                           ';
                            }
                 }else{
                  if($page_array[$count] == '...'){
                   ...
                   $page_link.= '
                   <li class="page-item">
                     <a class="page-link" href="/msearch?q='.$_GET['q'].'&page='.$page_array[$count].'">'.$page_array[$count].'</a>
                   </li>
                   ';
                    }
                  }
