...
if(count($url) > 4 || count($url) == 2){
 ...
 if(count($url) == 3){
   $hid = explode("-", $url[2]);
   $id = end($hid);
   $sec_count = countContent2($conn, "web_content", "section", $id);

   if($sec_count < 1){
     header("Location: /");
     exit();
    }else{
     $category = fetchRecentContentW2C($conn, "web_content", "section", $id, "visibility", "show", "web_content_id", 10);
     $page_title = $post_section_data[$id];
    }
   if($sec_count < 1){
     $page_title = $post_section_data[$id];
    }else{
...
if(count($url) == 4){
  $hid = explode("-", $url[3]);
  $id = end($hid);
  $cat_count = countContent2($conn, "web_content", "category_id", $id);

  if($cat_count < 1){
    $page_title = $post_category_data[$id];
  }else{
    $category = fetchRecentContentW2C($conn, "web_content", "category_id", $id, "visibility", "show", "web_content_id", 10);
    $page_title = $post_category_data[$id];
  }
}
     ...
     <div class="container">
       <div class="row">
         <div class="content col-lg-9">
           ...
           <?php if(isset ($sec_count)): ?>
             <div class="page-title">
              <?php $sec_name = fetchRecord2($conn, "web_section", "section_hash_id", $id) ?>
              <h1>All posts in - <?php echo $sec_name['web_section_name'] ?></h1>
              <div class="breadcrumb float-left">
                ...
                <li class="active">
                  <a href="/category/<?= $sec_name['sec_slug'] ?>-<?= $sec_name['section_hash_id'] ?>"><?php echo $sec_name['web_section_name'] ?>
                  </a>
                </li>
                </ul>
              </div>
           ...
           <?php else: ?>
             <div class="page-title">
              <?php $cat_name = fetchRecord2($conn, "web_category", "category_hash_id", $id) ?>
              <h1><?php echo $cat_name['web_category_name'] ?> - <?php echo $post_section_data[$cat_name['web_section_id']] ?></h1>
              <div class="breadcrumb float-left">
                <ul>
                <li><a href="/">Home</a>
                </li>
                <li><a href="/category/<?= $post_section_slug[$cat_name['web_section_id']] ?>-<?= $cat_name['web_section_id'] ?>"><?php echo $post_section_data[$cat_name['web_section_id']] ?></a>
                </li>
                <!-- <li><a href="#">Category</a>
                </li> -->
                <li class="active">
                  <a href="/category/<?= $post_section_slug[$cat_name['web_section_id']] ?>/<?= $cat_name['cat_slug'] ?>-<?= $cat_name['category_hash_id'] ?>"><?php echo $cat_name['web_category_name'] ?>
                  </a>
                </li>
                </ul>
              </div>
           ...
           <div id="blog" class="post-thumbnails">
             <div id="loadrecord">
              ...
              <?php else: ?>
                  ...
                  <div class="post-item">
                    <div class="post-item-wrap">
                      <h3>No post yet for this category</h3>
                    </div>
                  </div>

              <?php endif ?>
              </div>
              <?php if(isset ($sec_count) && $sec_count > 10): ?>
                <div id="showMore">
                  <a onclick="loadRecord('/ajax_sc', '<?= $id ?>')" class="btn btn-rounded btn-light"><i class="icon-refresh-cw"></i> Load More Posts</a>
                </div>
              <?php endif ?>
                <?php if(isset ($cat_count) && $cat_count > 10): ?>
                <div id="showMore">
                  <a onclick="loadRecord('/ajax_cat', '<?= $id ?>')" class="btn btn-rounded btn-light"><i class="icon-refresh-cw"></i> Load More Posts</a>
                </div>
              <?php endif ?>
