...
try{
  ...
  $prevImg = $conn->prepare("SELECT * FROM hom_mav WHERE hom_mav_id=:id");
  ...
  if (!in_array($imageNewExt, $img_allowed)){
    ...
    if(empty ($status['failed'])){
      ...
      $forum_image = $conn->prepare("UPDATE hom_mav SET hom_mav_image = :img WHERE hom_mav_id = :id");
      ...
      if(move_uploaded_file($imageTmpName, $imageDestination)){
        ...
        $_SESSION['msg'] = "Music Cover Image Changed Successfully";
      }
