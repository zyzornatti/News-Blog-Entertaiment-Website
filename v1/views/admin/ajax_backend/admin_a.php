...
$query .= 'ORDER BY admin_activities_id DESC ';
$filter_query = $query. 'LIMIT '.$start.', '.$limit.'';

$stmt = $conn->prepare($query);
$stmt->bindParam(":adm", $id);
$stmt->execute();
...
for ($count = 0; $count < count($page_array); $count++) {
  if($page == $page_array[$count]){
    ...
    if($previous_id > 0){
      $previous_link = '
        <li class="page-item">
          <a class="page-link" href="javascript:void(0)" onclick="loadThis('.$previous_id.', 10, '.$id.')" data-page="'.$previous_id.'">Previous</a>
        </li>
      ';
    }else{
      $previous_link = '
    ...
    if($next_id >= $total_links){
      ...
      $next_link = '
        <li class="page-item">
          <a class="page-link" href="javascript:void(0)" onclick="loadThis('.$next_id.', 10, '.$id.')" data-page="'.$next_id.'">Next</a>
        </li>
      ';
    }
    ...
    if($page_array[$count] == '...'){
      ...
      $page_link .= '
        <li class="page-item">
          <a class="page-link" href="javascript:void(0)" onclick="loadThis('.$page_array[$count].', 10, '.$id.')" data-page="'.$page_array[$count].'">'.$page_array[$count].'</a>
        </li>
      ';
    }
  }
