...
try{
  ...
  $prevImg = $conn->prepare("SELECT * FROM contact WHERE id=:id");
  ...
  if (!in_array($imageNewExt, $img_allowed)){
    ...
    if(empty ($status['failed'])){
      ...
      $forum_image = $conn->prepare("UPDATE contact SET contact_image = :img WHERE id = :id");
      ...
      if(move_uploaded_file($imageTmpName, $imageDestination)){
        ...
        $_SESSION['msg'] = "Contact Image Changed Successfully";
      }
